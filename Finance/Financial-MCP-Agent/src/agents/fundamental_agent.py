...
async def fundamental_agent(state: AgentState) -> AgentState:
    """
    Perform fundamental analysis using the ReAct framework, directly integrating MCP tools

    Args:
        state: the current Agent state containing the user query

    Returns:
        The updated AgentState, including the fundamental analysis result
    """
    logger.info(
    ...
    try:
        ...
        try:
            ...
            # Build a detailed fundamental analysis request, covering multiple analysis dimensions
            agent_input = f"""Please analyze the fundamentals of {company_name} (stock ticker: {stock_code}).

Current time: {current_time_info}
Current date: {current_date}

Please perform the following fundamental analysis:
1. Obtain the company's basic information and industry background
2. Obtain the latest financial statement data (balance sheet, income statement, cash flow statement)
3. Analyze profitability metrics (gross margin, net margin, ROE, etc.)
4. Analyze growth metrics (revenue growth rate, profit growth rate, etc.)
5. Analyze operating efficiency metrics (receivables turnover, inventory turnover, etc.)
6. Analyze solvency metrics (debt-to-asset ratio, current ratio, etc.)
7. Look up the historical dividend record
8. Provide a comprehensive fundamental assessment and investment value analysis

Important constraint: Please focus on financial data and fundamental metric analysis; do not use the crawl_news tool to obtain news information. Fundamental analysis should be based on financial statements, financial metrics, and company fundamental data, not on news events.

Please use the available tools to obtain actual data for the analysis, rather than relying on assumptions. If some data cannot be obtained, try using different time periods or other tool combinations, and provide as comprehensive an analysis as possible based on the available information.
...
async def test_fundamental_agent():
    """Test function for the fundamental analysis Agent"""
    from src.utils.state_definition import AgentState

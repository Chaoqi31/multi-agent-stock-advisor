...
from langchain_openai import ChatOpenAI  # Restore the OpenAI interface call
...
async def value_agent(state: AgentState) -> AgentState:
    """
    Perform valuation analysis using the ReAct framework, directly integrating MCP tools

    Args:
        state: the current Agent state containing the user query

    Returns:
        The updated AgentState, including the valuation analysis result
    """
    logger.info(
    ...
    try:
        ...
        try:
            ...
            # Build a detailed valuation analysis request, covering multiple analysis dimensions
            agent_input = f"""Please analyze the valuation of {company_name} (stock ticker: {stock_code}).

Current time: {current_time_info}
Current date: {current_date}

Please perform the following valuation analysis:
1. Obtain the company's basic information (market capitalization, share price, etc.)
2. Obtain and analyze the main valuation metrics (P/E ratio, P/B ratio, P/S ratio, etc.)
3. Compare the valuation metrics against the industry average
4. Analyze the trend of historical valuation levels
5. Obtain and analyze dividend data and dividend yield
6. Compute and analyze the intrinsic value
7. Provide a valuation summary and investment recommendation

Important constraint: Please focus on valuation metrics and financial data analysis; do not use the crawl_news tool to obtain news information. Valuation analysis should be based on financial metrics, valuation ratios, and historical data, not on news events.

Please use the available tools to obtain actual data for the analysis, rather than relying on assumptions. If some data cannot be obtained, try using different tools or parameter combinations, and provide as comprehensive an analysis as possible based on the available information. Please keep the answer concise and avoid lengthy descriptive text.
...
async def test_value_agent():
    """Test function for the valuation analysis Agent"""
    from src.utils.state_definition import AgentState

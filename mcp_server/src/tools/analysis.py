...
def register_analysis_tools(app: FastMCP, active_data_source: FinancialDataSource):
    """
    Register the analysis tools with the MCP app.
    ...
    def get_stock_analysis(code: str, analysis_type: str = "fundamental") -> str:
        """
        Provide a data-driven stock analysis report, not investment advice.

        Args:
            code: US stock ticker, e.g. 'AAPL'
            analysis_type: Analysis type; one of 'fundamental', 'technical', or 'comprehensive'

        Returns:
            A data-driven analysis report including key financial metrics, historical performance, and peer comparison
        """
        logger.info(
            f"Tool 'get_stock_analysis' called for {code}, type={analysis_type}")

        # Collect actual data across multiple dimensions
        try:
            # Get basic information
            basic_info = active_data_source.get_stock_basic_info(code=code)

            # Fetch different data depending on the analysis type
            if analysis_type in ["fundamental", "comprehensive"]:
                # Get the most recent quarter's financial data
                recent_year = datetime.now().strftime("%Y")
                recent_quarter = (datetime.now().month - 1) // 3 + 1
                if recent_quarter < 1:  # Handle the boundary case that may occur early in the year
            ...
            # Build an objective data analysis report
            ...
            try:
                ...
                logger.warning(f"Failed to retrieve industry comparison data: {e}")
